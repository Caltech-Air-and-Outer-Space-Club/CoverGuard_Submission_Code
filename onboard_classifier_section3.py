"""
CoverGuard-PFI  |  Section 3 — Onboard Failure-Cause Classifier
=================================================================
Paper section: §3 — Onboard Classification

Trains a LightGBM classifier on ~297k labeled pixels to diagnose cover-crop
failure causes from 8 spectral indices derived from 6 imager bands. Achieves
macro-F1 = 0.96 across 6 agronomic failure classes.

Inputs:
  - classifier_training_datasets/agriculture_dataset.csv  — 297k synthetic rows
  - classifier_training_datasets/merged_training_data.csv — optional merged dataset
    with real HLS pixels (generated by hls_data_pull.py)

Outputs:
  outputs/section3_onboard_classifier/
    coverguard_failure_classification_report.txt — per-class precision/recall/F1
  trained_model/
    coverguard_lgbm_classifier.txt               — serialised LightGBM booster

Run:
    python onboard_classifier_section3.py              # train on synthetic data
    python onboard_classifier_section3.py --merged     # train on synthetic + real HLS
    python onboard_classifier_section3.py --demo-only  # load saved model, run demo

Dependencies: lightgbm, scikit-learn, numpy, pandas. No NASA account required
              unless generating the merged dataset with hls_data_pull.py.
"""
...
# STEP 0 — FILE PATHS
# ══════════════════════════════════════════════════════════════════
# All paths are resolved relative to this file so the project is portable
# across machines without editing hard-coded directories.
...
# Class definitions, confidence threshold, spectral index list, class weights,
# per-class spectral profiles for data augmentation, and LightGBM hyperparameters.
FAILURE_CAUSES = {
    0: "HEALTHY",
    1: "MOISTURE_STRESS",
    2: "EXCESS_WETNESS",
...
# ══════════════════════════════════════════════════════════════════
# Compute all 8 agronomic spectral indices from 6 raw imager band reflectances.
# This is also the real on-orbit entry point: raw bands in → index features out.

def compute_spectral_indices(blue, green, red, red_edge, nir, swir):
    """
    Compute all spectral indices from 6 raw surface-reflectance band values (0–1).
    Covers the full HLS/HLS-VI index family supported by the CoverGuard imager.
    eps = 1e-9 prevents division-by-zero at dark pixels without altering results.

    Args:   blue, green, red, red_edge, nir, swir — scalar or array reflectances in [0, 1]
    Returns: dict mapping index name → computed value (same shape as inputs)
    """
    eps = 1e-9
    return {
...
# ══════════════════════════════════════════════════════════════════
# STEP 2 — AGRONOMIC LABEL DERIVATION
# ══════════════════════════════════════════════════════════════════
# Translate CSV proxy columns (soil moisture, temperature, canopy coverage, etc.)
# into the 6 failure-cause class labels using literature-backed rule thresholds.
# Rules are applied in priority order so rare classes are not masked by common ones.

def derive_failure_cause(df: pd.DataFrame) -> pd.Series:
    """
    Assign one of 6 failure-cause labels to each row using research-backed
    agronomic thresholds applied to proxy columns in the synthetic dataset.
    Classes are evaluated in rarity order (rarest first) so common classes
    cannot overwrite rare ones. THERMAL_STRESS was removed — temperature is
    not derivable from spectral bands alone and cannot be inferred on-orbit.

    Threshold sources:
      PEST_OR_DISEASE : Jackson et al. 1981 — biotic stress: moderate NDVI drop + high CSI
      POOR_EST.       : Kaspar et al. 2012; USDA-NRCS 340 — <20% canopy in early growth
      NUTRIENT_DEFICIT: Schepers 1996; Fitzgerald 2010; Brady & Weil 2010
      EXCESS_WETNESS  : Setter & Waters 2003 — anaerobic above ~33% VWC
      MOISTURE_STRESS : Gao 1996; FAO-56 — wilting near 15% VWC in loam soils

    Args:   df — DataFrame containing proxy columns (Soil_Moisture, Temperature, etc.)
    Returns: integer Series of class labels (0–5), aligned to df.index
    """
...
# ══════════════════════════════════════════════════════════════════
# Load the training CSV, derive or validate failure-cause labels, and augment
# synthetic rows with per-class Gaussian spectral profiles for NDMI, NDRE, EVI,
# NDWI, and MSAVI — indices absent from the synthetic dataset but needed by the model.

def load_and_prepare(path: Path):
    """
    Load the training CSV, derive failure-cause labels if absent, remap any
    legacy THERMAL_STRESS rows to HEALTHY, and augment missing spectral index
    columns with per-class Gaussian samples from SPECTRAL_PROFILES.

    Args:   path — Path to agriculture_dataset.csv or merged_training_data.csv
    Returns: (X DataFrame of feature columns, y Series of integer class labels)
    """
...
# ══════════════════════════════════════════════════════════════════
# Split data into 70/15/15 train/val/test stratified splits, train LightGBM
# with early stopping on the validation set, and return the fitted model.

def train_model(X, y):
    """
    Stratified 70/15/15 split, then train LightGBM with early stopping on the
    validation set to prevent overfitting on the large synthetic majority class.

    Args:   X — feature DataFrame; y — integer label Series
    Returns: (fitted LGBMClassifier, X_test DataFrame, y_test Series)
    """
...
# ══════════════════════════════════════════════════════════════════
# Generate per-class precision/recall/F1, confusion matrix, and feature
# importances on the held-out test set; save the text report to outputs/.

def evaluate_model(model, X_test, y_test):
    """
    Evaluate the trained model on the held-out test set and print a full
    classification report, confusion matrix, and top-12 feature importances.
    The text report is also saved to REPORT_PATH for submission artefacts.

    Args:   model — fitted LGBMClassifier; X_test, y_test — held-out split
    Returns: None (side-effects: prints to stdout, writes REPORT_PATH)
    """
...
# ══════════════════════════════════════════════════════════════════
# Serialise the trained LightGBM booster to a text file for onboard deployment
# and for reloading in --demo-only mode without retraining.
...
# ══════════════════════════════════════════════════════════════════
# STEP 7 — DEMO: ON-ORBIT INFERENCE
# ══════════════════════════════════════════════════════════════════
# CoverGuardClassifier wraps the trained model with two inference paths:
# the real onboard path (raw band reflectances → indices → class) and a
# debug path (pre-computed features → class). run_demo() exercises six
# agronomic scenarios to validate spectral separability end-to-end.
